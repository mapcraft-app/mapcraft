...
const MCP = require('../MCplugin'), MCplugin = new MCP();
...
class Cutscene
{
	...
	static GenerateCutscene(ID)
	{
		...
		let data = { file: '', camera: '', current_duration: 0, old_duration: 0 };
		const Join = (JSON) => {
			let data = '';
			for (let id in JSON)
			{
		...
		for (let id = 0; id < POINTS.length; id++)
		{
			...
			else
			{
				...
				data.camera += '# Point ' + POINTS[id].Point + '\n';
				for (let id = 0; TimeToPoint <= data.current_duration; id++)
				{
					let Teleport = [POINT(X.Points[id]), POINT(Y.Points[id]), POINT(Z.Points[id]), POINT(Rx.Points[id]), POINT(Ry.Points[id])].join('');
